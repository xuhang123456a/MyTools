...
function extractPlaylistId(urlLike) {
    ...
    for (const text of candidates) {
        ...
        if (parsedUrl) {
            const queryId = parsedUrl.searchParams.get("id") ||
                ...
                parsedUrl.searchParams.get("dissid");
            if (queryId && /^\d+$/.test(queryId)) {
                return queryId;
            }
            const pathId = (parsedUrl.pathname.match(/\/playlist\/(\d+)/) || [])[1];
            if (pathId) {
                return pathId;
            }
        }
        const queryMatch = (text.match(/[?&](?:id|disstid|dissid)=([0-9]+)/) || [])[1];
        if (queryMatch) {
            return queryMatch;
        }
        const pathMatch = (text.match(/\/playlist\/([0-9]+)/) || [])[1];
        if (pathMatch) {
            return pathMatch;
        }
    }
...
function parseQQJsonResponse(payload) {
...
async function requestMusicSheetById(id) {
    const baseParams = `type=1&json=1&utf8=1&onlysong=0&disstid=${id}&format=jsonp&g_tk=5381&loginUin=0&hostUin=0&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0`;
    const urls = [
    ...
    for (const url of urls) {
        try {
            ...
            const res = parseQQJsonResponse(result);
...
async function importMusicSheet(urlLike) {
    const id = extractPlaylistId(urlLike);
    if (!id) {
        return [];
    }
    const songList = await requestMusicSheetById(id);
